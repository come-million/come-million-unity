...
class Font6x8 {
    ...
    public static void Draw(Texture2D tex, int x, int y, char c, Color color) {
        int d = (int)c * 8;
        for(int j=0; j<8; j++) {
            var row = data[d + j];
            for(int i=0; i<=6; i++) {
                var col = row & (1 << i);
                if(col != 0)
                    tex.SetPixel(x + 3 - i, y + 4 - j, color);
            }
        }
    }

    public static void Draw(Texture2D tex, int x, int y, string s, Color color) {
        for(int i=0; i<s.Length; i++)
            Draw(tex, x + 6 * i, y, s[i], color);
    }
